...
Процедура ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитов, ТипЗначенияРеквизита, ИмяРеквизита, Значение) Экспорт
	
	ЗначениеРеквизита 	= ФабрикаXDTO.Создать(ТипЗначенияРеквизита);
...
Процедура ДобавитьЗначениеСвойстваXDTO(ЗначенияСвойств, ТипЗначенияСвойства, ИдСвойства, НаименованиеСвойства, Значение) Экспорт
	
	ЗначенияСвойства 	= ФабрикаXDTO.Создать(ТипЗначенияСвойства);
...
Функция ПолучитьСписокПолейПоСтроке(пСтрокаПолей) Экспорт 
...
//
Процедура ДобавитьАдресноеПолеXDTO(XDTOАдрес, XDTOАдресноеПолеТип, ТипПоля, Значение)
	
	XDTOАдресноеПоле			= ФабрикаXDTO.Создать(XDTOАдресноеПолеТип);
...
// Возвращаемое значение:
//   Объект XDTO "Контакт" 
Функция ПолучитьКонтактнуюИнформациюXDTO(XDTOКонтактТип, ВидКонтакта, ПредставлениеКонтакта) Экспорт

	XDTOКонтакт				= ФабрикаXDTO.Создать(XDTOКонтактТип);
...
Функция ОбновлениеДоВерсии6_5_0_0(ОсновныеПараметры)
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 6.5.0.0 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры);
		Возврат Ложь;
...
Функция ДобавитьОсновныеНастройкиВПараметры(УзелОбмена, Параметры, ПринудительнаяПолноеОбновлениеДанныхТовара = Ложь) Экспорт
	...
	СхемаXSD = ФабрикаXDTO.Пакеты.Получить("urn:1C.ru:commerceml_3");

	Параметры.Вставить("ПространствоИмен", СхемаXSD.URIПространстваИмен);
		           
	Параметры.Вставить("ВерсияСхемы"		, "3.1");
	Параметры.Вставить("ФайлСхемы"			, СхемаXSD);
	...
	Параметры.Вставить("ПлатформаWindows", ?(ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент(), Истина, Ложь));
...
Процедура ВыполнитьОбмен(Настройка, ПринудительнаяПолноеОбновлениеДанныхТовара = Ложь) Экспорт
	...
	Если НЕ Б_ОбщиеПроцедурыИФункцииСервер.КаталогОчищен(КаталогНаДиске) Тогда
	...
	СообщитьПодробно("Удаление временных файлов: " + ОсновныеПараметры.ФайлСхемы, ОсновныеПараметры, Истина, Ложь);	
