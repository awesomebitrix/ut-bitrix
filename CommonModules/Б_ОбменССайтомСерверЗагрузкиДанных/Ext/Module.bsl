...
Процедура ЗаполнитьИнформациюОЧеке(Документ, ДокументXML)
	...
	Если ИнформацияОЧеках <> Неопределено тогда
		...
		Если ИнформацияОЧеках.Количество() > 0 тогда
			...
			НоваяЗапись = РегистрыСведений.Б_ИнформацияОЧеках.СоздатьМенеджерЗаписи();
			НоваяЗапись.Документ 	= Документ; 
			НоваяЗапись.НомерЧека 	= лНомерЧека; 
			НоваяЗапись.Записать(Истина);
			
			Для Каждого ТекСвойство из НужныйЧек.ЗначенияСвойств Цикл
				НоваяЗапись = РегистрыСведений.Б_ИнформацияОЧеках.СоздатьМенеджерЗаписи();
				НоваяЗапись.Документ 	= Документ; 
				НоваяЗапись.НомерЧека 	= лНомерЧека; 
				НоваяЗапись.ИдСвойства 	= ТекСвойство.Ид; 
				НоваяЗапись.Значение 	= ТекСвойство.Значение; 
				НоваяЗапись.Записать(Истина);
			КонецЦикла;
			...
			Если Константы.Б_ИспользоватьМеханизмПечатиЧеков.Получить() И Документ.Проведен И Оплачено тогда
				...
				Если НайденнаяСтрока <> Неопределено тогда
					...
					Если НуженЧек тогда
						ДобавлениеОплатыВОчередьПечати(Документ);	
					КонецЕсли;
...
Процедура ДобавлениеОплатыВОчередьПечати(Документ)
...
Функция ПолучитьЗначениеРеквизитаДокументаПоСоответствию(ПараметрыОбмена, ДокументXML, Документ, ОсновныеНастройкиДокумента, ТекущийРеквизит, СтрокаXML = Неопределено, СтрокаТабличнойЧасти = Неопределено)
	...
	Если ТекущийРеквизит.ТипДанных = "Из поля XML документа" тогда
		...
		ИначеЕсли ТекущийРеквизит.Значение = "Адрес доставки" тогда
			...
			//УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияXMLПоПредставлению(ТекКонтакт.Представление, Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента)	
